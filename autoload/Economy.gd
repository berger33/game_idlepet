...
func upgrade_cost(level: int) -> float:
    return ceil(RemoteConfig.get_float("upgrade_cost_base") * pow(RemoteConfig.get_float("upgrade_cost_growth"), level))

func income_multiplier(level: int) -> float:
    return pow(RemoteConfig.get_float("upgrade_income_growth"), level)

func service_reward(base_reward: float, quality: StringName, upgrade_level: int, combo: int) -> float:
    ...
    return floor(base_reward * income_multiplier(upgrade_level) * quality_multiplier * combo_multiplier)
...
func offline_earnings(rate_per_second: float, elapsed_seconds: float, prestige_level: int) -> float:
    ...
    return floor(rate_per_second * clamped_seconds * RemoteConfig.get_float("offline_rate") * (1.0 + prestige_level * 0.05))
