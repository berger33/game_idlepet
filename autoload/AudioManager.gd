extends Node
## Áudio do jogo: música ambiente sintetizada em runtime (intocada) + SFX de
## alta qualidade pré-renderizados em assets/audio/sfx/ pelo tools/gen_sfx.py
## (modelagem física a 44.1 kHz: um instrumento diferente por ação).
##
## SFX — cada ação tem timbre próprio que evoca o gesto real:
##   * banho = bolhas d'água (glissando de Minnaert), tosa = tesourada com
##     anel de aço, secagem = sopro de ar rosa, laço = corda de nylon
##     dedilhada (Karplus-Strong), perfume = atomizador + sino de vidro;
##   * tudo em pentatônica de Dó maior: qualquer sequência permanece
##     consonante com o loop ambiente (C, Am, F, G) — errar não "briga";
##   * pool de 4 vozes (round-robin): eventos se sobrepõem em vez de cortar;
...
##   * "window" — harpa de vidro ao entrar na faixa do perfect: dá para
##     dosar o momento de soltar de ouvido.
##
## Regenerar/validar os assets: python tools/gen_sfx.py [--check].

## Taxa da síntese da MÚSICA em runtime (intocada). Os assets de SFX são
## 44.1 kHz e carregados prontos — Godot reamostra na reprodução.
...
## Onde vivem os WAVs gerados por tools/gen_sfx.py (44.1 kHz, 16 bits).
...
func _build_sfx_cache() -> void:
	# --- Escadas progressivas do gesto: 10 degraus por serviço. A nota (ou o
	# brilho, no secador) sobe com o progresso — o jogador OUVE que está perto.
...
func _play_entry(sfx: StringName, pitch: float, volume_scale: float = 1.0) -> void:
	if not cache.has(sfx):
		return
	var stream: AudioStreamWAV = cache[sfx]
	if stream == null:
		return
...
## Carrega um WAV de SFX pré-renderizado (assets/audio/sfx/NOME.wav).
...
## Escada progressiva: registra os N degraus (bubble_0..N-1...) + fallback.
