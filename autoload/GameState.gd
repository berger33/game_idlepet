...
func _on_service_completed(_service_id: StringName, quality: StringName, reward: float) -> void:
	...
	if services_completed >= 3 and not unlocked_pets.has("luna_shih_tzu"):
		unlocked_pets.append("luna_shih_tzu")
		EventBus.toast_requested.emit("Luna entrou na sua coleção!", Color("ff8fb1"))
