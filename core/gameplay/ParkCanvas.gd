...
func _draw() -> void:
	...
	for i: int in 3:
		...
		center.y -= hop
		var scale: float = 0.72
		if tex != null:
			var sz: Vector2 = Vector2(tex.get_width(), tex.get_height()) * scale
			var rect: Rect2 = Rect2(center - sz * 0.5, sz)
			draw_texture_rect(tex, rect, false)
			# sombra
			draw_ellipse(center + Vector2(0, 90), Vector2(80, 22), Color("263238", 0.18))
		# nome do pet (legível)
		var pname: String = ContentDB.pet_name(pid) if ContentDB.has_pet(pid) else pid.capitalize()
		var font: Font = FONT
		var fsz: int = 26
		draw_string(font, center + Vector2(-font.get_string_size(pname, HORIZONTAL_ALIGNMENT_CENTER, -1, fsz).x * 0.5, 140), pname, HORIZONTAL_ALIGNMENT_LEFT, -1, fsz, Color("263238", 0.95))
	# atividade overlay
	if activity == &"ball":
		# bolinha
		var ball_col: Color = Color("ff3d57") if ball_pos.distance_to(pet_focus(1)) < 110 else Color("ff6f8a")
		draw_circle(ball_pos, 46.0, ball_col)
		draw_circle(ball_pos, 42.0, Color.WHITE)
		draw_circle(ball_pos, 34.0, ball_col)
		draw_circle(ball_pos + Vector2(-10, -10), 10.0, Color.WHITE)
		# rastro demanda arrasto
		draw_dashed_line(ball_pos, pet_focus(1), Color.WHITE, 3.0, 12.0)
	elif activity == &"treat":
		for i: int in 3:
			var c: Vector2 = Vector2(260 + i * 280, 1050)
		...
		# viewfinder pips dos 3 pets
		for i: int in 3:
			var pip: Vector2 = Vector2(220 + i * 240, 1130) + Vector2(randf_range(-8, 8), randf_range(-6, 6)) * (1.0 - photo_align) * 8.0
			var ok: bool = photo_align > 0.72
			draw_circle(pip, 34.0, Color("a5d6a7") if ok else Color("ffcc80"))
			draw_circle(pip, 30.0, Color.WHITE)
